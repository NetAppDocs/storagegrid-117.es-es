...
Para obtener más información, consulte https://["Documentación de Amazon Web Services (AWS) sobre reglas de nomenclatura de bloques"^].
...
IMPORTANT: Una vez que se habilita la configuración Bloqueo de objetos S3 para un depósito, no se puede desactivar. Cualquier persona con los permisos correctos puede agregar objetos a este depósito que no se pueden cambiar. Es posible que no pueda eliminar estos objetos o el cubo en sí.
