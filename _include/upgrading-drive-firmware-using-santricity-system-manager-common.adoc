...
* Si está realizando una actualización sin conexión, es necesario link:../maintain/placing-appliance-into-maintenance-mode.html["Puso el dispositivo StorageGRID en modo de mantenimiento"].
+

NOTE: Mientras que en modo de mantenimiento se detiene la actividad de I/O en la controladora de almacenamiento para proteger las operaciones de almacenamiento disruptivas.




IMPORTANT: No actualice el firmware de la unidad en más de un dispositivo StorageGRID a la vez. De este modo, se puede provocar la falta de disponibilidad de los datos, dependiendo del modelo de puesta en marcha y la política del ciclo de vida de la información.

.Acerca de esta tarea
Puede actualizar el firmware de la unidad del dispositivo mediante dos métodos:

*En línea*: Las unidades se actualizan de una en una mientras el aparato está realizando operaciones de E/S (operaciones de entrada/salida). Este método no requiere que se ponga el dispositivo en modo de mantenimiento. Sin embargo, el rendimiento del sistema puede verse afectado y puede tardar varias horas más que el método sin conexión.
...
* Sin conexión*: Las unidades se actualizan en paralelo mientras el aparato está en modo de mantenimiento. Si el pool o el grupo de volúmenes no es compatible con la redundancia o se degrada, se debe utilizar el método sin conexión para actualizar el firmware de la unidad. El método sin conexión actualiza el firmware solo cuando se detiene toda la actividad de I/O en las unidades para actualizarse. Debe detener todas las operaciones de I/O de las unidades que pretenda actualizar. Para ello, coloque el nodo en modo de mantenimiento.
...
IMPORTANT: Hay dos tipos de unidades en el SG6060: SSD y HDD. Debe utilizar el método sin conexión para actualizar el firmware en los SSD. Puede utilizar el método en línea o sin conexión para actualizar el firmware de los HDD.

.Pasos
. Si está realizando una actualización sin conexión, confirme que el dispositivo está en link:../maintain/placing-appliance-into-maintenance-mode.html["modo de mantenimiento"].
...
IMPORTANT: Antes de utilizar este método, debe poner el aparato en modo de mantenimiento. Debe utilizar el método *Offline* para actualizar el firmware de la unidad.
