...
NOTE: Una regla de ILM no puede usar un *Tiempo de creación definido por el usuario* para el Tiempo de referencia y las opciones equilibradas o estrictas para el comportamiento de ingesta. Se devuelve un error cuando se crea la regla de ILM.
...
IMPORTANT: Como práctica recomendada general, incluya siempre estos encabezados en él `CanonicalHeaders` Para asegurarse de que se verifican; sin embargo, si excluye estas cabeceras, StorageGRID no devuelve un error.

Para obtener más información, consulte https://["Cálculos de firma para la cabecera de autorización: Transferencia de carga útil en un solo fragmento (AWS Signature versión 4)"^].
